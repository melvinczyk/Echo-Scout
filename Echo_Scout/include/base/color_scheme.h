...
// Each is a degree-6 polynomial in t, producing a continuous gradient

inline uint16_t sampleIron(float t) {  // Magma fit — black → purple → red → orange → white
...
inline uint16_t sampleViridis(float t) {  // dark purple → teal → green → yellow
...
inline uint16_t samplePlasma(float t) {  // dark blue → purple → pink → yellow
