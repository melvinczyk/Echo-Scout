...
static bool project(float wx, float wy, float wz, int& sx, int& sy) {
    ...
    qRot(ImuState::qR, -ImuState::qI, -ImuState::qJ, -ImuState::qK,
         wx, wy, wz, vx, vy, vz);
    float depth = -vy;
    if (depth < Map3dScreen::NEAR) return false;
    sx = Map3dScreen::VIEW_CX + (int)(Map3dScreen::FOCAL * vx / depth);
    sy = Map3dScreen::VIEW_CY + (int)(Map3dScreen::FOCAL * vz / depth);
    return sx >= 2 && sx < Display::SCREEN_W - 2 &&
           sy >= Map3dScreen::VIEW_Y + 2 && sy < Map3dScreen::VIEW_Y + Map3dScreen::VIEW_H - 2;
}
...
static bool zoneWorld(int zone, float dist,
    ...
    float ah =  (col - 3.5f) * Map3dScreen::ZONE_STEP;
    float av = -(row - 3.5f) * Map3dScreen::ZONE_STEP;
    float lx =  sinf(ah);
...
static void renderView() {
    ...
    Display::spr.createSprite(Display::SCREEN_W, Map3dScreen::VIEW_H);
    Display::spr.fillSprite(Display::Colors::BG);

    int count = min(nPts, Map3dScreen::MAX_PTS);
    int step = max(1, count / Map3dScreen::RENDER_MAX);
    for (int i = 0; i < count; i += step) {
        int sx, sy;
        if (project(pts[i].x, pts[i].y, pts[i].z, sx, sy))
            Display::spr.drawPixel(sx, sy - Map3dScreen::VIEW_Y, pts[i].col);
    }
    ...
    int cx = Map3dScreen::VIEW_CX, cy = Map3dScreen::VIEW_CY - Map3dScreen::VIEW_Y;
    ...
    Display::spr.pushSprite(0, Map3dScreen::VIEW_Y);
    Display::spr.deleteSprite();
}

static void drawButtons() {
    int by = Display::SCREEN_H - Map3dScreen::BTN_BAR;
    Display::tft.fillRect(0, by, Display::SCREEN_W, Map3dScreen::BTN_BAR, Display::Colors::BG);
    Display::tft.drawFastHLine(0, by, Display::SCREEN_W, Display::Colors::SEP);
    Display::tft.drawRect(Map3dScreen::CLR_X, Map3dScreen::BTN_Y, Map3dScreen::CLR_W, Map3dScreen::BTN_BH, Display::Colors::RED);
    Display::tft.setTextColor(Display::Colors::RED, Display::Colors::BG);
    Display::tft.drawCentreString("CLEAR", Map3dScreen::CLR_X + Map3dScreen::CLR_W/2, Map3dScreen::BTN_Y + 9, 2);
}
...
static void captureScan() {
    ...
    for (int z = 0; z < 64; z++) {
        float wx, wy, wz;
        if (!zoneWorld(z, TofState::distances[z], qr, qi, qj, qk, wx, wy, wz)) continue;
        pts[nPts % Map3dScreen::MAX_PTS] = { wx, wy, wz, ColorScheme::fromDist(cfg.mapPaletteIdx, TofState::distances[z]) };
        nPts++;
    }
}
...
void drawMap3dBase() {
    if (!pts)
        pts = (MapPoint*)heap_caps_malloc(Map3dScreen::MAX_PTS * sizeof(MapPoint), MALLOC_CAP_SPIRAM);
    nPts = 0;
...
void handleMap3dTouch(int tx, int ty) {
    if (inRect(tx, ty, Map3dScreen::CLR_X, Map3dScreen::BTN_Y, Map3dScreen::CLR_W, Map3dScreen::BTN_BH)) {
        nPts = 0;
        renderView();
    }
}
